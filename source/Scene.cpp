		...
		for (int i = 0; i < m_SphereGeometries.size(); i++)
		{
			...
			if (closestHit.didHit)
			{
				if (!firstHit)
				{
					...
					closeHitOrigin = closestHit.origin;
					closeHitNormal = closestHit.normal;
					firstHit = true;
				...
				{
					closestHit.origin = closeHitOrigin;
					closestHit.normal = closeHitNormal;
				}
			}
		}
		//Planes
...
#pragma region SCENE W3
	...
	{
		m_Camera.origin = { 0.f,3.f,-9.f };
		/*m_Camera.origin = { 0.f,1.f,-5.f };*///test
		m_Camera.fovAngle = 45.f;
		//default: Material id0 >> SolidColor Material (RED) // Test
		...
		//Light
		AddPointLight({ 0.f,5.f,5.f }, 25.f, ColorRGB{ 1.f,0.61f,0.45f });
...
#pragma region SCENE W4
	...
	void Scene_W4_BunnyScene::Initialize()
	{
		...
		// CW Winding Order!
		m_Mesh = AddTriangleMesh(TriangleCullMode::BackFaceCulling, matLambert_White);
		Utils::ParseOBJ("Resources/simple_cube.obj",
			m_Mesh->positions,
			m_Mesh->normals,
			m_Mesh->indices);

		//"Resources/lowpoly_bunny2.obj"

		m_Mesh->Scale({ 2.f,2.f,2.f });
		m_Mesh->Translate({ 0.f,1.f,0.f });
